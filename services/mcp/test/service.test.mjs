...
import { createMcpHandler } from "../src/http.mjs";
...
test("funnel → standalone profile → context → rules and durable execution history", async () => {
  ...
  try {
    assert.match(renderFunnelPage(), /https:\/\/mcp.bittrees.org\/connect/);
    assert.doesNotMatch(renderFunnelPage(), /client-config/);
