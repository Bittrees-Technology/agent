...
Build from the repository root with `node services/mcp/build.mjs`. The resulting
`output/mcp-service` is self-contained and has its own package lock, API entry,
Vercel configuration, catalog snapshot and schemas. Deploy it to project
`bittrees-mcp`, never the `agent` project. It imports no Agent contribution runtime
or Agent environment variables. No Node consumer is implemented.
...
Run `npm ci --prefix services/mcp`, `npm test --prefix services/mcp`, root checks,
and `node services/mcp/build.mjs`. Set `MCP_TEST_DATABASE_URL` to an isolated test
